...
EXIST_BLUE=$(sudo docker-compose -p ${DOCKER_APP_NAME}_blue -f docker-compose.blue.yml ps | grep Up)
...
if [ -z "$EXIST_BLUE" ]; then
  ...
  echo "blue 배포 시작 : $(date +%Y)-$(date +%m)-$(date +%d) $(date +%H):$(date +%M):$(date +%S)" >> /home/ubuntu/deploy.log
	...
	sudo docker-compose -p ${DOCKER_APP_NAME}_blue -f docker-compose.blue.yml up -d --build
  ...
  sudo docker-compose -p ${DOCKER_APP_NAME}_green -f docker-compose.green.yml down
...
# blue가 실행중이면 green up
	...
	sudo docker-compose -p ${DOCKER_APP_NAME}_green -f docker-compose.green.yml up -d --build

  sleep 30

  echo "blue 중단 시작 : $(date +%Y)-$(date +%m)-$(date +%d) $(date +%H):$(date +%M):$(date +%S)" >> /home/ubuntu/deploy.log
  sudo docker-compose -p ${DOCKER_APP_NAME}_blue -f docker-compose.blue.yml down
  sudo docker image prune -af
